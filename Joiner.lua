...
-- Sistema de spam ultra rÃ¡pido con prioridad dinÃ¡mica
...
_G.ChilliConfig = _G.ChilliConfig or {
    ...
    PriorityMode = "newest", -- "newest" = siempre el mÃ¡s reciente | "money" = mÃ¡s dinero
    ShowLogs = true,
    MaxQueueSize = 50
}
...
AJ.CurrentTarget = nil -- Servidor que estÃ¡ intentando
...
function handleMessage(jsonData)
    ...
    -- Actualizar target inmediatamente si es mÃ¡s nuevo
    updateTarget()
end
        ...
        while AJ.IsAttempting and _G.ChilliConfig.AutoJoinEnabled do
            ...
            if not success then
                ...
                if errorMsg:match("GameFull") or errorMsg:match("full") then
                    log("ðŸ”´ raiseTeleportInitFailedEvent: Teleport failed because Requested experience is full (GameFull)", "error")
                    log(string.format(
                        "âš ï¸ [AntiError] ðŸš« Intercepted teleport error: Enum.TeleportResult.GameFull Requested experience is full %d/%d",
                        target.players,
                        target.maxPlayers
                    ), "attempt")
                    log("â™»ï¸ [Retry] Servidor lleno, reintentando...", "attempt")
                    -- NO remover, seguir intentando
                
                -- Error: SERVIDOR CERRADO/EXPIRÃ“ (cambiar a otro)
                elseif errorMsg:match("GameEnded") or errorMsg:match("Could not find") then
                    log("âš ï¸ [Debug] TeleportInitFailed: Enum.TeleportResult.GameEnded", "error")
                    log("ðŸ”´ raiseTeleportInitFailedEvent: Teleport failed because Could not find requested game instance (GameEnded)", "error")
                    
                    -- Verificar si hay mÃ¡s servidores en cola
                    if #AJ.ServerQueue > 0 then
                        log(string.format("ðŸ”„ [Switch] Cambiando a otro servidor (%d en cola)", #AJ.ServerQueue), "attempt")
                        ...
                        log("â³ [Wait] No hay otros servidores, reintentando este...", "attempt")
                    end
                
                -- Error: TELEPORT EN PROCESO (esperar)
                elseif errorMsg:match("IsTeleporting") or errorMsg:match("processing") then
                    log("âš ï¸ [Debug] TeleportInitFailed: Enum.TeleportResult.IsTeleporting", "error")
                    log("ðŸ”´ raiseTeleportInitFailedEvent: Teleport failed because The previous teleport is in processing (IsTeleporting)", "error")
                    log("â±ï¸ [Wait] Esperando proceso anterior...", "attempt")
                    task.wait(1.5) -- Esperar mÃ¡s
                
                -- Error: OTROS
                else
                    log(string.format("âŒ [Error] %s", errorMsg), "error")
                    
                    -- Si es error 771 u otros similares
                    if errorMsg:match("771") or errorMsg:match("773") or errorMsg:match("timeout") then
                        log("ðŸ”„ [Network Error] Error de red, reintentando...", "attempt")
                    end
                end
            else
                log("âœ… [Success] ðŸŸ¢ Teleport iniciado correctamente", "success")
                log("â³ [Loading] Cargando servidor...", "success")
            end
...
local function createGUI()
    ...
    Title.Text = "ðŸŒ¶ï¸ CHILLI AUTO JOINER V3"
    ...
    task.spawn(function()
        while ScreenGui.Parent do
            StatusLabel.Text = "Estado: " .. (AJ.Connected and "âœ… Conectado" or "âŒ Desconectado")
...
print("â•‘   ðŸŒ¶ï¸  CHILLI AUTO JOINER V3     â•‘")
print("â•‘      AGGRESSIVE MODE - 0.1s      â•‘")
